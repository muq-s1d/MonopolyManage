...
export type Reply = { ok?: true; pending?: true; error?: string; pid?: string; token?: string; admin?: boolean }
...
export function createClient(send: (m: Msg) => void, o: Opts = {}) {
  ...
  let view: View = { game: null, entries: [], state: null, players: [], seated: [], offers: [], pid: null, token: o.token ?? null, admin: false, ended: false }
  ...
  return {
    ...
    receive(m: Msg) {
      switch (m.t) {
        case 'hello':
          return set({ game: m.game, entries: m.entries, state: m.game && replay(m.game, m.entries), players: m.players, seated: m.seated, offers: m.offers, ended: false })
        case 'sync': {
    ...
    act<K extends ActionName>(name: K | 'undo', ...args: ActionArgs<K>) {
